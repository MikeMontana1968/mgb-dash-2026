...
- **LED ring signal integrity issue** — single pixel displays correctly (pixel 0 = red at 12 o'clock, confirmed). Multiple pixels cause corruption: ghost LEDs, wrong colors, data degradation downstream. Diagnosis: missing 330Ω series resistor on data line between ESP32 GPIO14 and LED ring DIN. Also need 470µF 16V+ electrolytic cap across ring VCC/GND.
- **Color order confirmed** — `NEO_GRB + NEO_KHZ800` is correct for these WS2812B rings.
- **Debug firmware** — AMPS startup temporarily replaced with cardinal-point LED test (R/G/B/Y at 12/3/6/9) in infinite loop for hardware validation. Blocked on resistor/cap installation.
- **Pending** — solder 330Ω resistor (series on data line) + 470µF cap (across power) on LED ring, then re-run orientation test to confirm pixel-to-clock mapping and finalize blink constants. Remove debug code and restore normal self-test.
