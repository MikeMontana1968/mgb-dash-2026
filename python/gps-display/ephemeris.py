...
Ported from suncalcPy2-based prototype. Provides sunrise/sunset, moonrise/moonset,
sun position, and decimal-hour conversions for the GPS display and ambient light
computation.

Self-contained: only uses math, datetime, time, calendar (no external deps).
"""
...
def getTimes(date, lat, lng):
    ...
    for i in range(0, len(times)):
        t = times[i]
        Jset = getSetJ(t[0] * rad, lw, phi, dec, n, M, L)
        Jrise = Jnoon - (Jset - Jnoon)
        result[t[1]] = fromJulian(Jrise).strftime('%Y-%m-%d %H:%M:%S')
        result[t[2]] = fromJulian(Jset).strftime('%Y-%m-%d %H:%M:%S')

    return result


def getSunDates(date, lat, lng):
    lw = rad * -lng
    phi = rad * lat
