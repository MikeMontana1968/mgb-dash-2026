...
constexpr uint32_t CAN_ID_BODY_SPEED        = 0x711;  // Vehicle speed from hall sensor (authority source), 10 Hz
constexpr uint32_t CAN_ID_BODY_GEAR         = 0x712;  // Estimated gear from motor RPM / driveshaft RPM ratio, 2 Hz
constexpr uint32_t CAN_ID_BODY_ODOMETER     = 0x713;  // Odometer reading, 1 Hz
...
// ── GPS Module ────────────────────────────────────────────────────────
constexpr uint32_t CAN_ID_GPS_SPEED             = 0x720;  // GPS speed
constexpr uint32_t CAN_ID_GPS_TIME              = 0x721;  // GPS time (seconds since midnight UTC)
constexpr uint32_t CAN_ID_GPS_DATE              = 0x722;  // GPS date (days since epoch 2000-01-01)
constexpr uint32_t CAN_ID_GPS_LATITUDE          = 0x723;  // GPS latitude
constexpr uint32_t CAN_ID_GPS_LONGITUDE         = 0x724;  // GPS longitude
constexpr uint32_t CAN_ID_GPS_ELEVATION         = 0x725;  // GPS elevation
constexpr uint32_t CAN_ID_GPS_AMBIENT_LIGHT     = 0x726;  // Ambient light category from time relative to sunset
constexpr uint32_t CAN_ID_GPS_UTC_OFFSET        = 0x727;  // UTC offset in minutes (signed, e.g. -300=EST, -240=EDT)
